...
static const std::vector<std::string> FLUKE_MESSAGES = {
    "FLUKE", "ACTUAL FLUKE", "BRO WHAT", "FLUKE RUN INCOMING", "NO SHOT",
    "WHAT IS HAPPENING", "BRO TURNED IT ON", "FLUKE ACTIVATED", "CERTIFIED FLUKE",
    "LMAOOOO", "HES COOKING NOW", "FLUKE MERCHANT", "WHAT A TURNAROUND",
    "THIS IS INSANE", "PLOT TWIST", "FLUKE ALERT",
};

// Funny GD meme messages shown at the very start (0% before any progress)
static const std::vector<std::string> PREGAME_MESSAGES = {
    "is Society top 1?",
    "it's called balancing - Zoink 2026",
    "FIRE IN THE HOLE",
    "Check Steam",
    "bro really woke up today",
    "npesta reaction arc incoming",
    "swag route lets go",
    "is this rated??",
    "I hope this stream goes 3h",
    "bro really said let me suffer today",
    "the grind never stops",
    "this is literally just skill issue",
    "chat is he gonna do it",
    "not him loading into another attempt",
    "RobTop add practice mode to extreme demons challenge",
    "the dedication is crazy",
    "bro said today is the day",
    "I've seen this movie before",
    "certified GD moment",
    "bro diff",
    "skill issue speedrun any%",
    "he's built different fr",
    "the audacity to play this level",
    "average GD player experience",
    "chat what's the attempt count rn",
    "least obsessed GD player",
    "this level is actually unbeatable",
    "Pointercrate says hi",
    "the demonlist fears him",
    "bro said no life? say less",
};

// Extra "I WAS HERE" style messages for super go section
static const std::vector<std::string> I_WAS_HERE_MESSAGES = {
    "I WAS HERE",
    "I WAS HERE!!!",
    "I WAS HERE???!!!",
    "I WAS HEREEEEEEEEEE",
    ...
    "I WAS ACTUALLY HERE",
    "CLIP CLIP CLIP",
    "I WAS HERE (not clickbait)",
    "I WAS HERE AND I HAVE PROOF",
    "recording this for evidence",
    "I WAS HERE 🗣️🗣️",
};
...
class $modify(MyPlayLayer, PlayLayer) {
    struct Fields {
        ...
        bool m_pregameChatFired = false;
        bool m_beatLevelChatFired = false;
    };
...
public:
    ...
    void checkProgress(float dt) {
        ...
        // FLUKE DETECTION
        if (progress < fields->m_lowestPercent) fields->m_lowestPercent = progress;
        if (progress < 20.0f) fields->m_hadEarlyDeath = true;
        if (fields->m_hadEarlyDeath && !fields->m_flukeDetected && progress >= 50.0f && fields->m_lowestPercent < 15.0f) {
        ...
        // pregame meme messages before any progress
        if (progress == 0.0f && !fields->m_pregameChatFired) {
            fields->m_pregameChatFired = true;
            addChatMessage(chat(PREGAME_MESSAGES[rand() % PREGAME_MESSAGES.size()]));
        }
        if (progress > 0.1f) {
            fields->m_pregameChatFired = false; // reset for next attempt
        }
        ...
        if (fields->m_idleChatTimer >= fields->m_nextIdleDelay && !fields->m_isDeathSpamming) {
            // mix in pregame memes during early progress too
            if (progress < fields->holdPercent && rand() % 3 == 0) {
                addChatMessage(chat(PREGAME_MESSAGES[rand() % PREGAME_MESSAGES.size()]));
            } else {
        ...
        // super go
            ...
            fields->m_randomChatTimer += dt;
            if (fields->m_randomChatTimer >= fields->m_nextChatDelay) {
                // randomly pick between super go messages and I WAS HERE spam
                std::vector<std::string> messages;
                if (rand() % 3 == 0) {
                    // 1 in 3 chance: pure I WAS HERE spam
                    messages = {
                        chat(I_WAS_HERE_MESSAGES[rand() % I_WAS_HERE_MESSAGES.size()]),
                        chat(I_WAS_HERE_MESSAGES[rand() % I_WAS_HERE_MESSAGES.size()]),
                        chat(I_WAS_HERE_MESSAGES[rand() % I_WAS_HERE_MESSAGES.size()]),
                    };
                } else {
    ...
    void levelComplete() {
        ...
        if (fields->m_beatLevelChatFired) return;
        fields->m_beatLevelChatFired = true;

        std::string levelName = m_level ? m_level->m_levelName : "this level";
        int attempts = fields->att + 1;

        if (attempts <= 100) {
            // beat in under 100 attempts - special message!
            std::string attStr = std::to_string(attempts);
            std::vector<std::string> messages = {
                randomPlayer() + ": HE BEAT " + levelName + " IN ONLY " + attStr + " ATTEMPTS",
                randomPlayer() + ": JUST " + attStr + " ATTEMPTS ON " + levelName + "???",
                randomPlayer() + ": " + attStr + " ATTEMPTS?? BRO IS INSANE",
                randomPlayer() + ": " + levelName + " IN " + attStr + " ATTS NO WAY",
                randomPlayer() + ": ONLY " + attStr + " TRIES ON " + levelName + "???!!!",
                randomPlayer() + ": bro beat " + levelName + " in " + attStr + " attempts im crying",
            };
            addChatMessage(messages[rand() % messages.size()]);
            // spam a few more reactions
            addChatMessage(chat("WHAT THE SIGMA"));
            addChatMessage(chat("NO WAY THAT WAS " + attStr + " ATTEMPTS"));
        }
    }
    ...
    void resetLevel() {
        ...
        fields->m_pregameChatFired = false;
    }
};
